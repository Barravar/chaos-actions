...
from .utils import serialize, read_manifest_content
...
def create_chaos_experiment(client: LitmusClient, project_id: str, litmus_infra_id: str, experiment_manifest: str, tags: list[str] | None = None) -> dict:
  """
...
def wait_experiment_completion(client: LitmusClient, project_id: str, experiment_id: str, notify_id: str, poll_interval: int = POLL_INTERVAL_SECONDS, timeout_seconds: int = POLL_TIMEOUT_SECONDS) -> dict:
  ...
  # STEP 1: List the experiment runs by experiment ID
  # It is not expected to have multiple runs for the same experiment at the same time, but if that happens we get the most recent run sorted by time.
  ...
  try:
    ...
    if not experiment_runs:
      logger.error(f"No experiment runs found for experimentID {experiment_id}")
      raise LitmusGraphQLError(f"No experiment runs found for experimentID {experiment_id}")
    
    # Only the most recent run is evaluated
    run_id = experiment_runs[0].get("experimentRunID")
  ...
  # Variables for the GraphQL query to get experiment run status by run ID
  get_run_vars = {
    "projectID": project_id,
  ...
  while True:
    try:
      ...
      if phase not in RUNNING_PHASES:
        # Experiment has reached a terminal state - log detailed information
        logger.info("="*100)
        ...
        # Parse and display fault-level details from execution data
        if execution_data:
          try:
            ...
            logger.info("-"*100)
            logger.info(f"Total Faults: {experiment_run.get('totalFaults', 'N/A')}")
    ...
    # Calculate attempt number and cap the exponent to prevent extremely long waits
    attempt = int(elapsed_time // poll_interval)
    backoff_time = min(poll_interval * (2 ** min(attempt, MAX_BACKOFF_EXPONENT)), MAX_BACKOFF_SECONDS)
    logger.debug(f"Next poll in {backoff_time}s (attempt {attempt + 1})")
